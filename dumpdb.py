all_users = [
    i.to_json()
    for i in utils.FamilyTreeMember.get(author.id).span(
...
for user in all_users:
    ...
    if user["partner_id"]:
        data.append(
            "MERGE (U{0})-[:MARRIED_TO]->(U{1})".format(
                user["discord_id"], user["partner_id"]
            )
        )
    if user["parent_id"]:
        data.append(
            "MERGE (U{0})-[:CHILD_OF]->(U{1})".format(
                user["discord_id"], user["parent_id"]
            )
        )
        data.append(
            "MERGE (U{1})-[:PARENT_OF]->(U{0})".format(
                user["discord_id"], user["parent_id"]
            )
        )
