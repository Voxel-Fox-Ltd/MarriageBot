...
class MiscCommands(utils.Cog):
    ...
    async def github(self, ctx: utils.Context):
        ...
        await ctx.send(
            f"<{self.bot.config['command_data']['github']}>", embeddify=False
        )
    ...
    async def donate(self, ctx: utils.Context):
        ...
        await ctx.send(
            f"See `{ctx.prefix}perks` for more information!\n<{self.bot.config['command_data']['patreon']}>",
            embeddify=False,
        )
    ...
    async def invite(self, ctx: utils.Context):
        ...
        await ctx.send(
            f"<{self.bot.get_invite_link(embed_links=True, attach_files=True)}>",
            embeddify=False,
        )
    ...
    async def perks(self, ctx: utils.Context):
        ...
        e.add_field(
            name=f"Voting ({ctx.prefix}vote)",
            value="Gives you access to:\n* " + "\n* ".join(voting_perks),
            inline=False,
        )
        e.add_field(
            name=f"T1 Subscriber ({ctx.prefix}donate)",
            value="Gives you access to:\n* " + "\n* ".join(t1_donate_perks),
            inline=False,
        )
        e.add_field(
            name=f"T2 Subscriber ({ctx.prefix}donate)",
            value="Gives you access to:\n* " + "\n* ".join(t2_donate_perks),
            inline=False,
        )
        e.add_field(
            name=f"T3 Subscriber ({ctx.prefix}donate)",
    ...
    async def stats(self, ctx: utils.Context):
        ...
        embed.add_field(
            name="Average WS Latency", value=f"{(self.bot.latency * 1000):.2f}ms"
        )
        embed.add_field(
            name="Coroutines",
            value=f"{len([i for i in asyncio.Task.all_tasks() if not i.done()])} running, {len(asyncio.Task.all_tasks())} total.",
        )
        embed.add_field(name="Process ID", value=self.process.pid)
        embed.add_field(name="CPU Usage", value=f"{self.process.cpu_percent():.2f}")
        embed.add_field(
            name="Memory Usage",
            value=f"{self.process.memory_info()[0]/2**20:.2f}MB/{psutil.virtual_memory()[0]/2**20:.2f}MB",
        )
        ...
        embed.add_field(
            name="Uptime",
            value=f"{uptime[0]} days, {uptime[1]} hours, {uptime[2]} minutes, and {uptime[3]:.2f} seconds.",
        )
    ...
    async def clear(self, ctx: utils.Context):
        ...
        if ctx.channel.permissions_for(ctx.guild.me).manage_messages:
            ...
            _ = await ctx.channel.purge(
                limit=100, check=lambda m: m.author.id == self.bot.user.id, bulk=False
            )
    ...
    async def block(self, ctx: utils.Context, user_id: utils.converters.UserID):
        ...
        async with self.bot.redis() as re:
            await re.publish_json(
                "BlockedUserAdd", {"user_id": ctx.author.id, "blocked_user_id": user_id}
            )
    ...
    async def unblock(self, ctx: utils.Context, user: utils.converters.UserID):
        ...
        async with self.bot.redis() as re:
            await re.publish_json(
                "BlockedUserRemove", {"user_id": ctx.author.id, "blocked_user_id": user}
            )
    ...
    async def shard(self, ctx: utils.Context, guild_id: int = None):
        ...
        await ctx.send(
            f"The shard for server ID `{guild_id}` is `{(guild_id >> 22) % self.bot.shard_count}`. If all instances have `{len(self.bot.shard_ids)}` shards, that guild would be on instance `{((guild_id >> 22) % self.bot.shard_count) // len(self.bot.shard_ids)}`"
        )
