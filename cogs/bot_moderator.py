...
class ModeratorOnly(utils.Cog):
    ...
    async def cachename(self, ctx: utils.Context, user: utils.converters.UserID = None):
        ...
        await ctx.send(
            f"Saved the name `{user!s}` into the database for user ID `{user.id}`."
        )
    ...
    async def copyfamilytoguildwithdelete(
        self, ctx: utils.Context, user: utils.converters.UserID, guild_id: int
    ):
        ...
        await db.conn.copy_records_to_table(
            "parents", columns=["child_id", "parent_id", "guild_id"], records=parents
        )
        await db.conn.copy_records_to_table(
            "marriages", columns=["user_id", "partner_id", "guild_id"], records=partners
        )
    ...
    async def addship(
        ...
        user2: typing.Optional[discord.Member] = None,
        percentage: float = 0,
    ):
        """
    ...
    async def addserverspecific(
        self, ctx: utils.Context, guild_id: int, user_id: utils.converters.UserID
    ):
        """Adds a guild to the MarriageBot Gold whitelist"""

        async with self.bot.database() as db:
            await db(
                "INSERT INTO guild_specific_families (guild_id, purchased_by) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET purchased_by=excluded.purchased_by",
                guild_id,
                user_id,
            )
    ...
    async def getgoldpurchase(self, ctx: utils.Context, user: utils.converters.UserID):
        ...
        async with self.bot.database() as db:
            rows = await db(
                "SELECT * FROM guild_specific_families WHERE purchased_by=$1", user
            )
        if not rows:
            return await ctx.send(
                "That user has purchased no instances of MarriageBot Gold."
            )
        return await ctx.invoke(
            self.bot.get_command("runsql"),
            content="SELECT * FROM guild_specific_families WHERE purchased_by={}".format(
                user
            ),
        )
    ...
    @commands.bot_has_permissions(send_messages=True)
    async def recache(
        self, ctx: utils.Context, user: utils.converters.UserID, guild_id: int = 0
    ):
        ...
        async with self.bot.database() as db:
            parent = await db(
                "SELECT parent_id FROM parents WHERE child_id=$1 AND guild_id=$2",
                user,
                guild_id,
            )
            children = await db(
                "SELECT child_id FROM parents WHERE parent_id=$1 AND guild_id=$2",
                user,
                guild_id,
            )
            partner = await db(
                "SELECT partner_id FROM marriages WHERE user_id=$1 AND guild_id=$2",
                user,
                guild_id,
            )
    ...
    async def recachefamily(
        ...
        family = utils.FamilyTreeMember.get(user, guild_id).span(
            expand_upwards=True, add_parent=True
        )[:]
        for i in family:
            parent = await db(
                "SELECT parent_id FROM parents WHERE child_id=$1 AND guild_id=$2",
    ...
    async def forcemarry(
        self,
        ctx: utils.Context,
        user_a: utils.converters.UserID,
        user_b: utils.converters.UserID = None,
    ):
    ...
    async def forceadopt(
        self,
        ctx: utils.Context,
        parent: utils.converters.UserID,
        child: utils.converters.UserID = None,
    ):
    ...
    async def addvoter(self, ctx: utils.Context, user: utils.converters.UserID):
        ...
        async with self.bot.database() as db:
            await db(
                "INSERT INTO dbl_votes (user_id, timestamp) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET timestamp=$2",
                user,
                self.bot.dbl_votes[user],
            )
        await ctx.send("Consider it done.")
    ...
    @utils.checks.is_bot_support()
    @commands.bot_has_permissions(send_messages=True)
    async def createblogpost(
        self, ctx: utils.Context, url: str, title: str, *, content: str = None
    ):
        ...
        async with self.bot.database() as db:
            try:
                await db(
                    "INSERT INTO blog_posts VALUES ($1, $2, $3, NOW(), $4)",
                    url,
                    title,
                    content,
                    ctx.author.id,
                ...
                await db(
                    "UPDATE blog_posts SET url=$1, title=$2, body=$3, created_at=NOW(), author_id=$4 WHERE url=$1",
                    url,
                    title,
                    content,
                    ctx.author.id,
                ...
                verb = "Updated"
        await ctx.send(
            f"{verb} blog post: https://marriagebot.xyz/blog/{url}", embeddify=False
        )
    ...
    async def createredirect(self, ctx: utils.Context, code: str, redirect: str):
        ...
        await ctx.send(
            f"Created redirect: <https://marriagebot.xyz/r/{code}> :)", embeddify=False
        )
