...
class OwnerOnly(utils.Cog, command_attrs={"hidden": True}):
    ...
    async def message(
        self, ctx: utils.Context, user_id: utils.converters.UserID, *, content: str
    ):
    ...
    async def runsql(self, ctx: utils.Context, *, content: str):
        ...
        for row in [{i: i for i in column_headers}] + list(database_data):
            for header in column_headers:
                column_max_lengths[header] = max(
                    column_max_lengths[header], len(str(row[header]))
                )
        ...
        for row in [{i: i for i in column_headers}] + list(database_data):
            ...
            for header in column_headers:
                current_line += (
                    format(str(row[header]), f"<{column_max_lengths[header]}") + "|"
                )
    ...
    @commands.bot_has_permissions(send_messages=True)
    async def activity(
        self, ctx: utils.Context, activity_type: str, *, name: str = None
    ):
    ...
    async def status(self, ctx: utils.Context, status: str):
        ...
        await self.bot.change_presence(
            activity=self.bot.guilds[0].me.activity, status=status_object
        )
