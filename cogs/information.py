...
class Information(utils.Cog):
    @commands.command(
        aliases=["spouse", "husband", "wife", "marriage"], cls=utils.Command
    )
    @utils.cooldown.cooldown(1, 5, commands.BucketType.user)
    @utils.checks.bot_is_ready()
    @commands.bot_has_permissions(send_messages=True)
    async def partner(
        self, ctx: utils.Context, user: typing.Optional[utils.converters.UserID]
    ):
        ...
        async with self.bot.database() as db:
            if self.bot.is_server_specific:
                data = await db(
                    "SELECT * FROM marriages WHERE user_id=$1 AND guild_id<>0", user
                )
            else:
                data = await db(
                    "SELECT * FROM marriages WHERE user_id=$1 AND guild_id=0", user
        ...
        if timestamp:
            await ctx.send(
                f"`{user_name}` is currently married to `{partner_name}` (`{user_info._partner}`). They've been married since {timestamp.strftime('%B %d %Y')}."
            )
        else:
            await ctx.send(
                f"`{user_name}` is currently married to `{partner_name}` (`{user_info._partner}`)."
            )
    ...
    async def relationship(
        ...
        other: utils.converters.UserID = None,
    ):
    ...
    async def familysize(
        self, ctx: utils.Context, user: utils.converters.UserID = None
    ):
    ...
    async def treefile(self, ctx: utils.Context, root: utils.converters.UserID = None):
        ...
        async with ctx.channel.typing():
            text = await utils.FamilyTreeMember.get(
                root_user_id, ctx.family_guild_id
            ).generate_gedcom_script(self.bot)
        file_bytes = io.BytesIO(text.encode())
        await ctx.send(
            file=discord.File(file_bytes, filename=f"tree_of_{root_user_id}.ged")
        )
    ...
    async def familytree(
        self, ctx: utils.Context, root: utils.converters.UserID = None
    ):
    ...
    @utils.checks.bot_is_ready()
    @commands.bot_has_permissions(send_messages=True, attach_files=True)
    async def stupidtree(
        self, ctx: utils.Context, root: utils.converters.UserID = None
    ):
        """Gets the family tree of a given user"""

        try:
            return await self.treemaker(ctx=ctx, root_user_id=root, stupid_tree=True)
        except Exception as e:
            raise e
    ...
    async def treemaker(
        ...
        async with ctx.channel.typing():
            if stupid_tree:
                ...
                dot_code = await tree.to_dot_script(
                    self.bot, None if all_guilds else ctx.guild, ctu
                )
        ...
        file = discord.File(
            fp=f'{self.bot.config["tree_file_location"]}/{ctx.author.id}.png'
        )
