...
class RedisHandler(utils.Cog):
    ...
    def __init__(self, bot: utils.Bot):
        ...
        self.handlers = [
            task(
                self.channel_handler(
                    "DBLVote",
                    lambda data: bot.dbl_votes.__setitem__(
                        data["user_id"],
            ...
            task(
                self.channel_handler(
                    "ProposalCacheAdd", lambda data: bot.proposal_cache.raw_add(**data)
                )
            ),
            task(
                self.channel_handler(
            ...
            task(
                self.channel_handler(
                    "BlockedUserAdd",
                    lambda data: bot.blocked_users[data["user_id"]].append(
            ...
            task(
                self.channel_handler(
                    "BlockedUserRemove",
                    lambda data: bot.blocked_users[data["user_id"]].remove(
                        data["blocked_user_id"]
                    ),
                )
            ),
            ...
            task(
                self.channel_handler(
                    "UpdateFamilyMaxMembers", self.update_max_family_members
                )
            ),
            task(
                self.channel_handler("UpdateIncestAllowed", self.update_incest_alllowed)
            ),
        ...
        self.handlers.extend(
            [
                task(
                    self.channel_handler(
                        "TreeMemberUpdate", lambda data: utils.FamilyTreeMember(**data)
                    )
                ),
            ]
        )
    ...
    def cog_unload(self):
        ...
        for channel in self._channels.copy():
            asyncio.ensure_future(
                asyncio.wait_for(
                    self.bot.redis.pool.unsubscribe(channel), timeout=None
                ),
                loop=self.bot.loop,
            )
    ...
    async def channel_handler(
        self, channel_name: str, function: callable, *args, **kwargs
    ):
        ...
        while await channel.wait_message():
            ...
            self.bot.redis.logger.debug(
                f"Received JSON at channel {channel_name}:{json.dumps(data)}"
            )
            try:
                if asyncio.iscoroutine(function) or asyncio.iscoroutinefunction(
                    function
                ):
