...
class BotSettings(utils.Cog):
    ...
    async def prefix(self, ctx: utils.Context, *, new_prefix: str):
        ...
        async with self.bot.database() as db:
            await db(
                f"INSERT INTO guild_settings (guild_id, {prefix_key}) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET {prefix_key}=$2",
                ctx.guild.id,
                new_prefix,
            )
    ...
    async def setup(self, ctx: utils.Context):
        ...
        menu.bulk_add_options(
            ctx,
            {
                "display": lambda c: "Set setting (currently {0})".format(
                    settings_mention(c, "setting_id")
                ),
                "converter_args": [
                    (
                ...
                "callback": utils.SettingsMenuOption.get_set_guild_settings_callback(
                    "guild_settings", "setting_id"
                ),
            },
        )
        try:
    ...
    async def usersettings(self, ctx: utils.Context):
        ...
        menu.bulk_add_options(
            ctx,
            {
                ...
                "callback": utils.SettingsMenuOption.get_set_user_settings_callback(
