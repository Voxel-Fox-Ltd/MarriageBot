...
class Parentage(utils.Cog):
        ...
        if self.bot.is_server_specific:
            ...
            if guild_max_children:
                gold_children_amount = max(
                    [
                        amount if int(role_id) in user._roles else 0
                        for role_id, amount in guild_max_children.items()
                    ]
                )

        # See how many children they're allowed normally (in regard to Patreon tier)
        normal_children_amount = await utils.checks.has_donator_perks_predicate(
            self.bot, "max_children", user
        )
                ...
                max(
                    [
                        ...
                        min(
                            [
                                i["max_children"]
                                for i in self.bot.config["role_perks"].values()
                            ]
                        ),
                    ]
                ),
                max(
                    [i["max_children"] for i in self.bot.config["role_perks"].values()]
                ),
            ]
        )
    ...
    async def makeparent(
        self, ctx: utils.Context, *, target: utils.converters.UnblockedMember
    ):
        ...
        if ctx.original_author_id not in self.bot.owner_ids:
            ...
            async with ctx.channel.typing():
                if (
                    instigator_tree.family_member_count
                    + target_tree.family_member_count
                    > max_family_members
                ):
                    return await ctx.send(
    ...
    async def disown(self, ctx: utils.Context, *, target: utils.converters.UserID):
        ...
        # Manage output strings
        text_processor = utils.random_text.RandomText(
            "disown", ctx.author, self.bot.get_user(target)
        )
    ...
    async def emancipate(self, ctx: utils.Context):
        ...
        # Manage output strings
        text_processor = utils.random_text.RandomText(
            "emancipate", ctx.author, self.bot.get_user(instigator_tree._parent)
        )
