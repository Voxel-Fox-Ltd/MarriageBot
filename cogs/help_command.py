...
class CustomHelpCommand(commands.MinimalHelpCommand):
    ...
    async def filter_commands(
        self, commands: typing.List[utils.Command]
    ) -> typing.List[utils.Command]:
    ...
    async def send_command_help(self, command: utils.Command):
        ...
        if self.include_invite:
            data.update(
                {"content": self.context.bot.config["command_data"]["guild_invite"]}
            )
        await self.send_to_destination(**data)

    async def send_bot_help(
        self,
        mapping: typing.Dict[typing.Optional[utils.Cog], typing.List[commands.Command]],
    ):
        ...
        for cog, cog_commands in runnable_commands.items():
            ...
            command_strings.append(
                (getattr(cog, "get_name", lambda: cog.name)(), value)
            )
    ...
    def get_help_line(self, command: utils.Command):
        ...
        if command.short_doc:
            return (
                f"{self.clean_prefix}{command.qualified_name} - *{command.short_doc}*"
            )
...
class Help(utils.Cog):
    def __init__(self, bot: utils.Bot):
        ...
        bot.help_command = CustomHelpCommand(dm_help=True, include_invite=False)
