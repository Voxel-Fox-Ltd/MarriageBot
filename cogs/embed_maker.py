...
class EmbedMaker(utils.Cog, command_attrs={"hidden": True}):
    ...
    async def makeembed(self, ctx, *, data: str = None):
        ...
        if data:
            ...
            return await self.send_embed(
                ctx, ctx.author, json_data.get("content"), json_data.get("embed", {})
            )
        ...
        while True:
            ...
            check = (
                lambda r, u: r.message.channel == instruction_message.channel
            ...
            if data is not None:
                ...
                try:
                    await user.send(
                        content,
                        embed=discord.Embed.from_dict(new_embed) if embed else None,
                    )
                    embed = new_embed
                except Exception as e:
                    await user.send(
                        f"Found an error sending that preview, reverting data - {e!s}"
                    )
                    await user.send(
                        content, embed=discord.Embed.from_dict(embed) if embed else None
                    )
    ...
    async def embed_set_content(self, user: discord.User, content: str, embed: dict):
        ...
        await user.send(
            "What do you want to set the **message content** as (`skip` to remove)?"
        )
        message = await self.bot.wait_for(
            "message", check=lambda m: m.channel == user.dm_channel and not m.author.bot
        )
    ...
    async def embed_add_field(self, user: discord.User, content: str, embed: dict):
        ...
        inline_message = await self.bot.wait_for(
            "message", check=lambda m: m.channel == user.dm_channel and not m.author.bot
        )
        embed["fields"].append(
            {
    ...
    async def embed_set_author(self, user: discord.User, content: str, embed: dict):
        ...
        icon_url_message = await self.bot.wait_for(
            "message", check=lambda m: m.channel == user.dm_channel and not m.author.bot
        )
        await user.send("What's the **URL** for this field (or `skip`)?")
        url_message = await self.bot.wait_for(
            "message", check=lambda m: m.channel == user.dm_channel and not m.author.bot
        )
        embed["author"] = {
            "name": name_message.content,
            "icon_url": icon_url_message.content
            if icon_url_message.content.lower() != "skip"
            else None,
            "url": url_message.content
            if url_message.content.lower() != "skip"
            else None,
        }
        return content, embed
    ...
    async def embed_set_description(
        self, user: discord.User, content: str, embed: dict
    ):
    ...
    async def embed_set_colour(self, user: discord.User, content: str, embed: dict):
        ...
        await user.send(
            "What **hex colour** do you want to set your embed to (`skip` to remove)?"
        )
        colour_message = await self.bot.wait_for(
            "message", check=lambda m: m.channel == user.dm_channel and not m.author.bot
        )
    ...
    async def send_embed(
        self, ctx: utils.Context, user: discord.User, content: str, embed: dict
    ):
        ...
        try:
            ...
            return await user.send(
                "I can't work out where you want to send that. Sorry about that. Cancelled."
            )
        if channel is None:
            return await user.send(
                "Found an error sending that. Sorry about that. Cancelled."
            )
