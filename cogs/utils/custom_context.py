...
class CustomContext(commands.Context):
    ...
    def _set_footer(self, embed: discord.Embed) -> discord.Embed:
        ...
        try:
            possible_footer_objects = [
                [i] * i.get("amount", 1) for i in self.bot.config["embed"]["footer"]
            ]  # Get from config
            footer_text_amount = []  # Make list for text
            [
                footer_text_amount.extend(i) for i in possible_footer_objects
            ]  # Flatten list
            footer_text = [
                {"text": i["text"]} for i in footer_text_amount
            ]  # Remove 'amount'
        ...
        footer["text"] = (
            ...
            .replace(f"<@!{self.bot.user.id}> ", f"<@{self.bot.user.id}> ")
            .replace(f"<@{self.bot.user.id}> ", f"@{self.bot.user!s} ")
        )
    ...
    async def send(
        ...
        *,
        ...
        try:
            channel_permissions: discord.Permissions = self.channel.permissions_for(
                self.guild.me
            )
            missing_permissions = not self.DESIRED_PERMISSIONS.is_subset(
                channel_permissions
            )
        ...
        if embed is None and embeddify:
            # Set content
            embed = discord.Embed(
                description=content, colour=random.randint(1, 0xFFFFFF)
            )
            ...
            if author_data:
                ...
                if author_name and author_url:
                    ...
                    embed.set_author(
                        name=author_name, icon_url=self.bot.user.avatar_url
                    )
