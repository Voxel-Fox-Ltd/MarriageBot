...
class SettingsMenuOption(object):
    ...
    async def convert_prompted_information(
        ...
        reactions: typing.List[discord.Emoji] = list(),
    ) -> typing.Any:
        ...
        try:
            if reactions:
                user_message = None
                check = (
                    lambda r, u: r.message.id == bot_message.id
                    and u.id == self.context.author.id
                    and str(r.emoji) in reactions
                )
                reaction, _ = await self.context.bot.wait_for(
                    "reaction_add", timeout=120, check=check
                )
                content = str(reaction.emoji)
            else:
                check = (
                    lambda m: m.channel.id == self.context.channel.id
                    and m.author.id == self.context.author.id
                )
    ...
    def get_guild_settings_mention(
        cls, ctx: commands.Context, attr: str, default: str = "none"
    ):
    ...
    def get_settings_mention(
        cls, ctx: commands.Context, settings: dict, attr: str, default: str = "none"
    ):
    ...
    def get_set_guild_settings_callback(
        ...
        async def callback(self, data):
            ...
            async with self.context.bot.database() as db:
                await db(
                    "INSERT INTO {0} (guild_id, {1}) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET {1}=$2".format(
    ...
    def get_set_user_settings_callback(
        ...
        async def callback(self, data):
            ...
            async with self.context.bot.database() as db:
                await db(
                    "INSERT INTO {0} (user_id, {1}) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET {1}=$2".format(
                        table_name, database_key
                    ),
                    self.context.author.id,
                    data,
                )
            self.context.bot.user_settings[self.context.author.id][
                database_key
            ] = original_data
    ...
    def get_set_iterable_delete_callback(
        ...
        async def callback(self, *data):
            ...
            try:
                ...
                self.context.bot.guild_settings[self.context.guild.id][cache_key].pop(
                    key_id
                )
    ...
    def get_set_iterable_add_callback(
        ...
        cache_key: str,
        database_key: str,
        serialize_function: typing.Callable[[typing.Any], str] = lambda x: x,
    ):
        """Return an async method that takes the data retuend by convert_prompted_information and then
        saves it into the database - should be used for the add_option stuff in the SettingsMenu init
        ...
        async def callback(self, *data):
            ...
            # Cache it
            if value:
                self.context.bot.guild_settings[self.context.guild.id][cache_key][
                    role.id
                ] = serialize_function(original_value)
            else:
                if (
                    role.id
                    not in self.context.bot.guild_settings[self.context.guild.id][
                        cache_key
                    ]
                ):
                    self.context.bot.guild_settings[self.context.guild.id][
...
class SettingsMenu(object):
    ...
    async def start(
        ...
        *,
        timeout: float = 120,
        clear_reactions_on_loop: bool = False,
    ):
        ...
        while True:
            ...
            try:
                check = (
                    lambda r, u: u.id == ctx.author.id and r.message.id == message.id
                )
...
class SettingsMenuIterable(SettingsMenu):
    ...
    def __init__(
        ...
        value_converter: commands.Converter = str,
        value_prompt: str = None,
        value_serialize_function: typing.Callable = None,
        *,
        iterable_add_callback: typing.Callable = None,
        iterable_delete_callback: typing.Callable = None,
    ):
        super().__init__()
        ...
        self.iterable_add_callback = (
            iterable_add_callback or SettingsMenuOption.get_set_iterable_add_callback
        )
    ...
    def get_sendable_data(self, ctx: commands.Context):
        ...
        if isinstance(data_points, dict):
            self.items = [
                SettingsMenuOption(
                    ctx,
                    f"{self.key_display_function(i)} - {self.value_converter(o)!s}",
                    (),
                    self.iterable_delete_callback(
                        self.database_name,
                        self.column_name,
    ...
    async def start(self, *args, clear_reactions_on_loop: bool = True, **kwargs):
        return await super().start(
            *args, clear_reactions_on_loop=clear_reactions_on_loop, **kwargs
        )
