...
class CustomCommand(commands.Command):
    def __init__(self, *args, **kwargs):
        ...
        if cooldown is None:
            ...
            mapping = (
                cooldown.default_mapping_class()
            )  # Get the default mapping from the object
        else:
    ...
    def get_remaining_cooldown(
        self, ctx: commands.Context, current: float = None
    ...
    def _prepare_cooldowns(self, ctx: commands.Context):
        ...
        if self._buckets.valid:
            ...
            try:
                ...
                ctx.bot.logger.critical(
                    f"Invalid cooldown set on command {ctx.invoked_with}"
                )
...
class CustomGroup(commands.Group):
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
            cooldown_after_parsing=kwargs.get("cooldown_after_parsing", True),
            **kwargs,
        )
        self.ignore_checks_in_help = kwargs.get("ignore_checks_in_help", False)
