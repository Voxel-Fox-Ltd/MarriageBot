...
def get_prefix(bot, message: discord.Message):
    ...
    try:
        settings = bot.guild_settings.get(
            message.guild.id, bot.DEFAULT_GUILD_SETTINGS.copy()
        )
...
class CustomBot(commands.AutoShardedBot):
    ...
    def __init__(
        self, *args, config_file: str, logger: logging.Logger = None, **kwargs
    ):
        ...
        self.DEFAULT_GUILD_SETTINGS = {
            ...
            "max_family_members": self.config[
                "max_family_members"
            ],  # Set in init; only used in gold
        ...
        super().__init__(
            ...
            guild_subscriptions=self.is_server_specific,
            *args,
            **kwargs,
        )
        ...
        self.shallow_users: typing.Dict[
            int, ShallowUser
        ] = {}  # A shallow copy of users' names
        ...
        # Set up some caches
        self.server_specific_families: typing.List[
            int
        ] = []  # List of whitelisted guild IDs
        ...
        self.blocked_users: typing.Dict[
            int, typing.List[int]
        ] = collections.defaultdict(
            list
        )  # uid: [blocked uids]
        self.guild_settings: typing.Dict[int, dict] = collections.defaultdict(
    ...
    async def startup(self):
        ...
        for row in text_lines:
            random_text.RandomText.original.all_random_text[row["command_name"]][
                row["event_name"]
            ].append(row["string"])
        ...
        try:
            all_settings = await db(
                "SELECT * FROM guild_settings WHERE (guild_id >> 22) % $1=ANY($2::INTEGER[])",
                self.shard_count,
                self.shard_ids,
            )
        except Exception as e:
        ...
        for items in all_settings:
            current_settings = self.guild_settings[
                items["guild_id"]
            ]  # Get current (which should include defaults)
        ...
        if self.is_server_specific:
            try:
                max_children_data = await db(
                    "SELECT * FROM max_children_amount WHERE (guild_id >> 22) % $1=ANY($2::INTEGER[])",
                    self.shard_count,
                    self.shard_ids,
                )
            except Exception as e:
    ...
    async def get_all_table_data(self, db, table_name):
        ...
        return await self.run_sql_exit_on_error(
    ...
    def get_invite_link(
        ...
        **kwargs,
        ...
        data = {
            "client_id": self.config.get("oauth", {}).get("client_id", None)
            or self.user.id,
            "scope": scope,
    ...
    async def add_delete_button(
        ...
        valid_users: typing.List[discord.User],
        *,
        delete: typing.List[discord.Message] = None,
        timeout=60.0,
    ):
    ...
    async def fetch_support_guild(self):
        ...
        self.support_guild = self.get_guild(
            self.config["guild_id"]
        ) or await self.fetch_guild(self.config["guild_id"])
    ...
    def get_max_family_members(self, guild: discord.Guild) -> int:
        ...
        return (
            self.guild_settings[guild.id]["max_family_members"]
            if self.is_server_specific
            else self.config["max_family_members"]
        )
    ...
    def allows_incest(self, guild: typing.Union[discord.Guild, int]) -> bool:
        ...
        return (
            self.is_server_specific
            and guild_id in self.guild_settings
            and self.guild_settings[guild_id]["allow_incest"]
        )
        ...
        if self.shard_count > 1:
            ...
            if shard_id:
                min, max = (
                    shard_id,
                    shard_id + 1,
                )  # If we're only setting it for one shard
            else:
                min, max = (
                    self.shard_ids[0],
                    self.shard_ids[-1],
                )  # If we're setting for all shards
            ...
            for i in range(min, max):
                activity = discord.Activity(
                    ...
                    type=getattr(
                        discord.ActivityType, presence["activity_type"].lower()
                    ),
                )
                status = getattr(discord.Status, presence["status"].lower())
                try:
                    await self.change_presence(
                        activity=activity, status=status, shard_id=i
                    )
                except KeyError:
                    self.logger.info(
                        f"Encountered KeyError on setting presence in shard {i}"
                    )
