...
class ErrorHandler(utils.Cog):
    async def send_to_ctx_or_author(
        self, ctx: utils.Context, text: str, author_text: str = None
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"discord.HTTPException on sending error message - {e.response}"
                )
    ...
    async def on_command_error(self, ctx: utils.Context, error: commands.CommandError):
        ...
        ignored_errors = (
            ...
            utils.errors.InvokedMetaCommand,
        )
        ...
        owner_reinvoke_errors = (
            ...
            utils.errors.NotBotModerator,
            utils.errors.NotBotSupport,
            utils.errors.NotServerSpecific,
        )
        ...
        # Voter
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                "You need to vote on DBL (`m!vote`) to be able to run this command.",
            )
        ...
        # Donator
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                "You need to be a donator (`m!perks`) to be able to run this command.",
            )
        ...
        # Not a server specific bot moderator
            ...
            if self.bot.is_server_specific:
                return await self.send_to_ctx_or_author(
                    ctx,
                    "You're missing the `MarriageBot Moderator` role required for this command.",
                )
            return await self.send_to_ctx_or_author(
                ctx, "This instance of the bot is not set to server specific."
            )
        ...
        # Not a bot administrator
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                "You need to be registered as MarriageBot support to run this command.",
            )
        ...
        # Not set to server specific
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                "You need to be using MarriageBot Gold to run this command - see (`m!perks`).",
            )
        ...
        # Missing argument (string)
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                f"You're missing the `{error.param}` argument, which is required for this command to work properly.",
            )
        ...
        # Did the quotemarks wrong
            ...
            return await self.send_to_ctx_or_author(
                ctx, "The quotes in your message have been done incorrectly."
            )
        ...
        # Missing argument
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                f"You're missing the `{error.param.name}` argument, which is required for this command to work properly.",
            )
        ...
        # Cooldown
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                f"You can't use this command again for another {utils.TimeValue(error.retry_after).clean_spaced}.",
            )

        # NSFW channel
        elif isinstance(error, commands.NSFWChannelRequired):
            return await self.send_to_ctx_or_author(
                ctx, "This command can't be run in a non-NSFW channel."
            )
        ...
        # User is missing a role
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                f"You need to have one of the {', '.join(['`' + i + '`' for i in error.missing_roles])} roles to run this command.",
            )
        ...
        # Bot is missing a given permission
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                f"I'm missing the `{error.missing_perms[0]}` permission, which is needed for me to run this command.",
            )
        ...
        # Missing permission
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                f"You need the `{error.missing_perms[0]}` permission to run this command.",
            )
        ...
        # Missing role
            ...
            return await self.send_to_ctx_or_author(
                ctx,
                f"You need to have the `{error.missing_role}` role to run this command.",
            )

        # Guild only
        elif isinstance(error, commands.NoPrivateMessage):
            return await self.send_to_ctx_or_author(
                ctx, "This command can't be run in DMs."
            )

        # DMs only
        elif isinstance(error, commands.PrivateMessageOnly):
            return await self.send_to_ctx_or_author(
                ctx, "This command can only be run in DMs."
            )

        # Not owner
        elif isinstance(error, commands.NotOwner):
            return await self.send_to_ctx_or_author(
                ctx, "You need to be registered as an owner to run this command."
            )
        ...
        # Discord hecked up
            ...
            try:
                return await self.send_to_ctx_or_author(
                    ctx,
                    f"Discord messed up there somewhere - do you mind trying again? I received a {error.status} error.",
                )
            except Exception:
                return
        ...
        try:
            ...
            if getattr(self.bot, "config", {}).get("dm_uncaught_errors", False):
                for owner_id in self.bot.config["owners"]:
                    ...
                    data.seek(0)
                    await owner.send(
                        error_text, file=discord.File(data, filename="error_log.py")
                    )
    ...
    async def tree_timeout_handler(self, ctx: utils.Context, error):
        ...
        if self.bot.is_server_specific:
            ...
            cooldown_time = await utils.checks.has_donator_perks_predicate(
                self.bot, "tree_cooldown_time", ctx.author
            )
        ...
        await ctx.send(
            f"You can only use this command once every {cooldown_display} (see `m!perks` for more information) per server. You may use this again in {time_remaining:.1f} seconds."
        )
