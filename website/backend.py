...
async def unblock_user_post_handler(request: Request):
    ...
    async with request.app["redis"]() as re:
        await re.publish_json(
            "BlockedUserRemove",
            {"user_id": logged_in_user, "blocked_user_id": blocked_user},
        )
...
async def set_prefix(request: Request):
    ...
    async with request.app["redis"]() as re:
        ...
        redis_data[
            {True: "gold_prefix", False: "prefix"}[bool(post_data["gold"])]
        ] = prefix
        await re.publish_json("UpdateGuildPrefix", redis_data)

    # Redirect to page
    location = (
        f"/guild_settings?guild_id={guild_id}&gold=1"
        if post_data["gold"]
        else f"/guild_settings?guild_id={guild_id}"
    )
...
async def set_max_family_members(request: Request):
    ...
    async with request.app["database"]() as db:
        await db(
            "INSERT INTO guild_settings (guild_id, max_family_members) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET max_family_members=$2",
            int(guild_id),
            max_members,
        )
    async with request.app["redis"]() as re:
...
async def set_gifs_enabled(request: Request):
    ...
    async with request.app["database"]() as db:
        await db(
            "INSERT INTO guild_settings (guild_id, gifs_enabled) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET gifs_enabled=$2",
            int(guild_id),
            enabled is True,
        )
    async with request.app["redis"]() as re:
...
async def set_incest_enabled(request: Request):
    ...
    async with request.app["database"]() as db:
        await db(
            "INSERT INTO guild_settings (guild_id, allow_incest) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET allow_incest=$2",
            int(guild_id),
            enabled,
        )
    async with request.app["redis"]() as re:
...
async def set_max_allowed_children(request: Request):
    ...
    max_children_data = {
        int(i): min([max([int(o), hard_minimum_children]), hard_maximum_children])
        for i, o in post_data.items()
        if i.isdigit() and len(o) > 0
    }  # user ID: amount
...
async def paypal_purchase_complete(request: Request):
    ...
    if (
        ...
        != request.app["config"]["payment_info"]["authorization"]
    ):
...
async def webhook_handler(request: Request):
    ...
    if (
        request.headers.get("Authorization", None)
        != request.app["config"]["topgg_authorization"]
    ):
        return Response(400)
    data = await request.json()
    time = dt.utcnow()
    ...
    async with request.app["redis"]() as re:
        await re.publish_json(
            "SendUserMessage", {"user_id": data["user_id"], "content": text}
        )
        await re.publish_json(
            "DBLVote", {"user_id": data["user_id"], "datetime": time.isoformat()}
        )

    # DB vote
    async with request.app["database"]() as db:
        await db(
            "INSERT INTO dbl_votes (user_id, timestamp) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET timestamp=excluded.timestamp",
            data["user_id"],
            time,
        )
    return Response(status=200)
