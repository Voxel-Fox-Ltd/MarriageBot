...
async def user_settings(request: Request):
    ...
    if len(request.query) > 0:
        ...
        data = await db(
            "SELECT * FROM customisation WHERE user_id=$1", session["user_id"]
        )
        try:
    ...
    blocked_users_db = await db(
        "SELECT blocked_user_id FROM blocked_user WHERE user_id=$1", session["user_id"]
    )
    blocked_users = {
        i["blocked_user_id"]: await request.app["bot"].get_name(i["blocked_user_id"])
...
async def guild_picker(request: Request):
    ...
    async with request.app["database"]() as db:
        gold_guild_data = await db(
            "SELECT * FROM guild_specific_families WHERE guild_id=ANY($1::BIGINT[])",
            guild_ids,
        )
...
async def guild_settings_get_paypal(request: Request):
    ...
    async with request.app["database"]() as db:
        ...
        ) or [request.app["bot"].DEFAULT_GUILD_SETTINGS.copy()]

        # Get gold allowance
        if not gold_param:
            gold_settings = await db(
                "SELECT * FROM guild_specific_families WHERE guild_id=$1", int(guild_id)
            )

        # Get disabled commands
        disabled_commands = {
            i: False for i in request.app["config"]["disableable_commands"]
        }
        disable_data = await db(
            "SELECT * FROM disabled_commands WHERE guild_id=$1", int(guild_id)
        )
        ...
        max_children_data = await db(
            "SELECT * FROM max_children_amount WHERE guild_id=$1", int(guild_id)
        )
    ...
    try:
        ...
        prefix = request.app["gold_config" if gold_param else "config"]["prefix"][
            "default_prefix"
        ]
    ...
    channels = sorted(
        [
            i
            for i in await guild_object.fetch_channels()
            if isinstance(i, discord.TextChannel)
        ],
        key=lambda c: c.position,
    )
    roles = sorted(
        [i for i in await guild_object.fetch_roles()],
        key=lambda c: c.position,
        reverse=True,
    )
    ...
    page_data = {
        ...
        "max_family_members": guild_settings[0][
            "max_family_members"
        ],  # Maximum amount of family members
        "allow_incest": guild_settings[0][
            "allow_incest"
        ],  # Whether incest is allowed or not
        ...
        "gifs_enabled": guild_settings[0][
            "gifs_enabled"
        ],  # Children amounts for this guild
        "max_children_hard_cap": max(
            [i["max_children"] for i in request.app["config"]["role_perks"].values()]
        ),  # Hard cap on children for all users
        "min_children_hard_cap": min(
            [i["max_children"] for i in request.app["config"]["role_perks"].values()]
        ),  # Hard minimum on children for all users
    }
    return page_data
